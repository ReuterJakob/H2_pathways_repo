...
params = {'font.size':9,
'font.weight':'normal',
'font.family':'arial', "figure.autolayout":True,'figure.constrained_layout.use' : True
    }
...
Grey_GWP100 = Direct_emissions_grey + Drive_CO2_emissions_grams_grey + Indirect_emissions_NOR_base + upstream_emissions_GWP100_low
Grey_GWP100

Grey_GWP20 = Direct_emissions_grey + Drive_CO2_emissions_grams_grey + Indirect_emissions_NOR_base + upstream_emissions_GWP20_NOR_Policy
Grey_GWP20

pess = Direct_emissions_low + Drive_CO2_emissions_grams_grey + Indirect_emissions_NOR_base + upstream_emissions_GWP20_NOR_Policy
pess

opt = Direct_emissions_high + Drive_CO2_emissions_grams_blue_flue_high + Indirect_emissions_NOR_base + upstream_emissions_GWP20_NOR_Policy
opt

"""## Plot total emissions for discussion"""
...
#asthetics
plt.axhline(RED_tresh_MJ, 0.5, color = 'lime', marker = 'o', label = 'EU Comission Treshold')
...
Upstream_emissions = [upstream_emissions_GWP100_NOR_Base, upstream_emissions_GWP20_NOR_Base, upstream_emissions_GWP100_NOR_Policy, upstream_emissions_GWP20_NOR_Policy,
                      ...
                      upstream_emissions_GWP20_NOR_Base, upstream_emissions_GWP20_NOR_Policy]
x =['Grey, GWP100\n0.02%', 'Grey, GWP20\n0.02%', 'Grey, GWP100\n1.3%', 'Grey, GWP20\n1.3%',
    'Baseline 2025', 'Policy 2025',
    'Baseline 2035','Policy 2035']
...
plt.show()
'''
# Emissions baseline stackplot
##read data for stackplot
...
title = '\Blue_emission_breakdown_NOR_base'
plt.savefig(path_plt + title + '.png', transparent=True)
plt.show()
'''
...
def lh2_transport_distance(p_el_y, alpha_liq, alpha_et, alpha_ship, alpha_it, alpha_recon,capex_liq_y, capex_et_y,capex_ship_y, capex_it_y,  bog_ship, capex_recon_y, d_sea_sensi ,v_ship, f_ship, el_et ,el_it ,t_it ,el_recon_y):
    ...
    for H2_emissions in cargo_emissions_range:
...
lh2_transport_sensi_cargo_emissions_LCOT_pol= pd.DataFrame(sensitivity, cargo_emissions_range, columns=['LH2 transport costs [€/kg H2]'])
...
lh2_transport_sensi_cargo_emissions_LCOT_base= pd.DataFrame(sensitivity, cargo_emissions_range, columns=['LH2 transport costs [€/kg H2]'])
...
def nh3_transport_sensi(p_el_y_sensi, alpha_con, alpha_et, alpha_ship, alpha_it, alpha_recon,capex_con_y, capex_et_y,capex_ship_y, capex_it_y,  bog_ship_nh3, capex_recon_y, d_sea_sensi ,v_ship, f_ship_nh3, el_et ,el_it ,t_it ,el_recon_y):
    ...
    for H2_emissions in cargo_emissions_range:
        ...
        result = \
            ...
            + (bog_ship_nh3 * d_sea_sensi/v_ship + f_ship_nh3 * d_sea_sensi) * (policy.NH3_costs_y + H2_emissions * P_co2_y_sensi / 1000 ) \
...
nh3_transport_sensi_cargo_emissions_LCOT_pol= pd.DataFrame(sensitivity, cargo_emissions_range, columns=['NH3 transport costs [€/kg H2]'])
...
nh3_transport_sensi_cargo_emissions_LCOT_base= pd.DataFrame(sensitivity, cargo_emissions_range, columns=['NH3 transport costs [€/kg H2]'])
...
plt.legend()
title = '\Transport_sensi_cargo_emissions_LCOT'
plt.savefig(path_plt + title + '.png', transparent=True)
...
#Lh2 baseline LEOT
CE = Conversion_emissions_LH2()
EE = ET_emissions_LH2()
LH2_cargo_ghg = CE + EE + Q_ue_y * 1000
SE = calculate_ship_emissions_LH2()
IE = IT_emissions_LH2()
RE = calculate_recon_emissions_LH2()
...
plt.plot(x, Q_ue_y*1000, marker = 'o',color = 'blue', label = 'LEOH Blue Baseline')
...
EE_plt = plt.bar(x ,EE, color = 'cornflowerblue', bottom= CE)
CE_plt = plt.bar(x ,CE, color = 'darkblue')
plt.plot(x, policy.Q_ue_y*1000, marker = 'D', color = 'blue', label = 'LEOH Blue Policy')
#Nh3 baseline LEOT
el_con_y = float(tea_NH3.loc['Conversion - Electricity consumption opt. [kWh/kgH2]'][year])
el_recon_y = float(tea_NH3.loc['Reconversion - Electricity consumption opt. [kWh/kg H2]'][year])
CE = Conversion_emissions_NH3()
EE = ET_emissions_NH3()
NH3_cargo_ghg = CE + EE + Q_ue_y * 1000
SE = calculate_ship_emissions_NH3()
IE = IT_emissions_NH3()
RE = calculate_recon_emissions_NH3()
x = 2
RE_plt = plt.bar(x ,RE,  color = 'orange', bottom= CE + EE + SE + IE)
IE_plt = plt.bar(x ,IE,  color = 'wheat', bottom= CE + EE + SE)
SE_plt = plt.bar(x ,SE,  color = 'crimson', bottom= CE + EE)
...
IC = policy.IC_US_NH3
RC = policy.RC_US_NH3
plt.bar(x,RC , color= 'orange', label='Reconversion costs',bottom= CC + EC + SC + IC)
plt.bar(x,IC , color = 'wheat', label='Import terminal costs', bottom= CC + EC + SC)
plt.bar(x,SC ,color = 'crimson', label='Shipping costs', bottom= CC + EC)
plt.bar(x,EC , color = 'cornflowerblue', label='Export terminal costs', bottom= CC)
plt.bar(x,CC , color = 'darkblue', label='Conversion costs')
