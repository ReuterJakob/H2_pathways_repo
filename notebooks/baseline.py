...
# Emissions treshholds in kgCO2eq/kg H2
RED_tresh_MJ = 28.3
#Certifhy_MJ = 36.4
RED_tresh_kg = 3.4
#Certifhy_kg = 4.368
...
## Plot price inputs.
co2_prices_baseline = prices.loc['CO2 prices [€/t_CO2] Baseline', 2010:2050]
co2_prices_policy = prices.loc['CO2 prices [€/t_CO2] Policy',2010:2050]
el_prices_baseline_nor = prices.loc['Electricity prices in Norway [€_2021/MWh] Baseline', 2010:2050]
el_prices_policy_nor = prices.loc['Electricity prices in Norway [€_2021/MWh] Policy',2010:2050]
el_prices_baseline_ger = prices.loc['Electricity prices in Germany [€_2020/MWh] Baseline', 2010:2050]
el_prices_policy_ger = prices.loc['Electricity prices in Germany [€_2020/MWh] Policy',2010:2050]
...
plt.plot(el_prices_policy_ger, color='gold', linestyle= '--',label='Policy')
plt.plot(el_prices_baseline_nor, color= 'orange', linestyle= '-',label= 'Electricity NOR Baseline')
plt.plot(el_prices_policy_nor, color='orange', linestyle= '--',label='Policy')
plt.plot(gas_prices_baseline_nor, color='blue', linestyle= '-',label='Gas NOR Baseline')
plt.plot(gas_prices_policy_nor, color='blue', linestyle= '--',label='Policy')
...
# Emission inputs
##read
EF_GER_base = GHG.loc['Grid electricity emission factor GER [g CO2eq/kWh] Baseline', 2010:2050]
EF_GER_pol = GHG.loc['Grid electricity emission factor GER [g CO2eq/kWh] Policy',2010:2050]
EF_NOR_base = GHG.loc['Grid electricity emission factor NOR [g CO2eq/kWh] Baseline', 2010:2050]
EF_NOR_pol = GHG.loc['Grid electricity emission factor NOR [g CO2eq/kWh] Policy',2010:2050]

Blue_em_NOR_base = GHG.loc['Blue hydrogen emissions [kg CO2/kg H2] - NOR Baseline', 2010:2050]
...
plt.plot(EF_NOR_pol, color='orange', linestyle= '--',label= 'Policy')
plt.grid(True, axis = 'y')
plt.xlim(2010,2050)
plt.ylim(0,)
ax.set_axisbelow(True)
plt.ylabel('Grid electricity emission factor $[g CO_2eq/kWh]$')
plt.legend()
...
Total_CO2_emissions = Total_CO2_emissions_grey + Indirect_emissions_NOR_base
...
Total_emissions_grey_GWP20_low = Total_CO2_emissions_grey + Indirect_emissions_NOR_base + upstream_emissions_GWP20_NOR_Policy
Total_emissions_grey_GWP20_low

# Total Emissions for grey hydrogen: CO2 + fugitive CH4 [g CO2e/MJ]
Total_emissions_grey_GWP20_mid = Total_CO2_emissions_grey + Indirect_emissions_NOR_base + upstream_emissions_GWP20_mid
Total_emissions_grey_GWP20_mid

# Total Emissions for grey hydrogen: CO2 + fugitive CH4 [g CO2e/MJ]
Total_emissions_grey_GWP20_high = Total_CO2_emissions_grey + Indirect_emissions_NOR_base + upstream_emissions_GWP20_high
...
Direct_emissions = [Direct_emissions_grey, Direct_emissions_grey, Direct_emissions_grey, Direct_emissions_grey,
...
Indirect_emissions_plt = [Indirect_emissions_NOR_base, Indirect_emissions_NOR_base, Indirect_emissions_US_base, Indirect_emissions_US_base,
                          Indirect_emissions_NOR_base, Indirect_emissions_NOR_base, Indirect_emissions_US_base, Indirect_emissions_US_base,
                          Indirect_emissions_NOR_base, Indirect_emissions_NOR_base, Indirect_emissions_US_base, Indirect_emissions_US_base]

Upstream_emissions = [upstream_emissions_GWP20_NOR_Base, upstream_emissions_GWP20_NOR_Policy, upstream_emissions_GWP20_mid,  upstream_emissions_GWP20_high,
...
fig, ax = plt.subplots(figsize=(8,4), frameon=False, layout = 'constrained')
width = 0.5
...
indirect = plt.bar(x, Indirect_emissions_plt, width, color ='grey', label ='CO2 from NG infrastructure', bottom=Upstream_emissions)
upstream = plt.bar(x, Upstream_emissions, width, color = 'lightcoral',label = 'CH4 from fugitive upstream methane')
#asthetics
plt.axhline(RED_tresh_MJ, 0, color = 'lime', marker = 'o', label = 'EU Comission Treshold')
...
ax2.set_ylim(0,36)
ax2.set_ylabel(ylabel= 'LEOH $[kg CO_2eq/kg H_2]$')
plt.xticks(rotation = 90)
plt.legend(bbox_to_anchor = (0,1.0,1.0,0), loc = 'lower left', mode = 'expand', ncol=2)
...
#US emissions
Direct_emissions = [Direct_emissions_high] * 4

Drive_emissions_plt = [Drive_CO2_emissions_grams_blue_flue_high] * 4

Indirect_emissions_plt = [Indirect_emissions_NOR_base, Indirect_emissions_NOR_base,
                          Indirect_emissions_US_base,Indirect_emissions_US_base]


Upstream_emissions = [upstream_emissions_GWP20_NOR_Base, upstream_emissions_GWP20_NOR_Policy,
                    upstream_emissions_GWP20_mid, upstream_emissions_GWP20_high]
...
#y_axis = np.arange(101,step=10)
direct = plt.bar(x, Direct_emissions,  color = 'royalblue', label='CO2 from SMR', bottom=list(map(lambda x, y, z: x + y + z, Upstream_emissions, Indirect_emissions_plt, Drive_emissions_plt)))
drive = plt.bar(x, Drive_emissions_plt,  color = 'dodgerblue',label='CO2 from NG combustion to drive SMR', bottom=list(map(lambda x, y: x + y, Upstream_emissions, Indirect_emissions_plt)))
indirect = plt.bar(x, Indirect_emissions_plt,  color ='grey', label ='CO2 from NG infrastructure', bottom=Upstream_emissions)
upstream = plt.bar(x, Upstream_emissions,  color = 'lightcoral',label = 'CH4 from fugitive upstream methane')
#asthetics
plt.axhline(RED_tresh_MJ, 0, color = 'lime', marker = 'o', label = 'EU Comission Treshold')
#plt.axhline(Certifhy_MJ, 0.5, color = 'lime', marker = 'D', label = 'CertifHy Treshold')
plt.grid(True, axis = 'y')
...
plt.legend()
#plt.title('GHG emissions from blue hydrogen production', fontweight = 'bold')
title = '\Blue_emission_breakdown_hbar'
plt.savefig(path_plt + title + '.png', transparent=True)
plt.show()
...
Direct_emissions = [Direct_emissions_high, Direct_emissions_high]

Drive_emissions_plt = [Drive_CO2_emissions_grams_blue_flue_high, Drive_CO2_emissions_grams_blue_flue_high]

Indirect_emissions_plt = [Indirect_emissions_US_base,Indirect_emissions_US_base]


Upstream_emissions = [upstream_emissions_GWP20_mid, upstream_emissions_GWP20_high]
...
Indirect_emissions_plt = [Indirect_emissions_NOR_base] * 2
...
#plt.axhline(Certifhy_MJ, 0.5, color = 'lime', marker = 'D', label = 'CertifHy Treshold')
plt.grid(True, axis = 'y')
ax3.set_ylabel(ylabel= ('LEOH [g CO2eq/MJ H2]'))
ax3.yaxis.set_major_locator(mtick.LinearLocator(6))
ax3.set_ylim(0,125)
ax3.set_axisbelow(True)
ax4 = ax3.secondary_yaxis('right', functions=(lambda MJ: MJ*120/1000, lambda kg: kg/120))
ax4.yaxis.set_major_locator(mtick.LinearLocator(6))
ax4.set_ylim(0,15)
ax4.set_ylabel(ylabel= 'LEOH $[kg CO_2eq/kg H_2]$')
plt.xticks()
plt.legend()
plt.title('NOR')
title = '\Blue_emission_breakdown_US_NOR'
plt.savefig(path_plt + title + '.png', transparent=True)
plt.show()
...
# PLot NOR emissions
Direct_emissions = [Direct_emissions_grey, Direct_emissions_grey, Direct_emissions_grey, Direct_emissions_grey,
                    Direct_emissions_high, Direct_emissions_high,
                    Direct_emissions_high, Direct_emissions_high]

Drive_emissions_plt = [Drive_CO2_emissions_grams_grey, Drive_CO2_emissions_grams_grey, Drive_CO2_emissions_grams_grey,
                       Drive_CO2_emissions_grams_grey, Drive_CO2_emissions_grams_grey, Drive_CO2_emissions_grams_grey,
                       Drive_CO2_emissions_grams_blue_flue_high, Drive_CO2_emissions_grams_blue_flue_high]

Indirect_emissions_plt = [Indirect_emissions_NOR_base] * 8
...
Upstream_emissions = [upstream_emissions_GWP100_NOR_Base, upstream_emissions_GWP20_NOR_Base, upstream_emissions_GWP100_NOR_Policy, upstream_emissions_GWP20_NOR_Policy,
                      upstream_emissions_GWP20_NOR_Base, upstream_emissions_GWP20_NOR_Policy,
                      upstream_emissions_GWP20_NOR_Base, upstream_emissions_GWP20_NOR_Policy]
x =['0.02%\nGWP100', '0.02%\nGWP20', '1.3%\nGWP100', '1.3%\nGWP20',
    '0.02%\n95% SGC', '1.3%\n95% SGC',
    '0.02%\n95% SGC\n90% FGC','1.3%\n95% SGC\n90% FGC']
...
plt.axhline(RED_tresh_MJ,0.5, color = 'lime', linestyle = '-', marker = 'o', label = 'EU Comission Treshold')
plt.grid(True, axis = 'y')
ax.set_ylabel(ylabel= ('LEOH [g CO2eq/MJ H2]'))
ax.yaxis.set_major_locator(mtick.LinearLocator(6))
...
ax2.set_ylim(0,15)
ax2.set_ylabel(ylabel= 'LEOH $[kg CO_2eq/kg H_2]$')
plt.legend(bbox_to_anchor = (0,1.02,1,0), loc = 'lower left', mode = 'expand', ncol=2)
#plt.title('GHG emissions from blue hydrogen production', fontweight = 'bold')


title = '\Blue_emission_breakdown_NOR'
plt.savefig(path_plt + title + '.png', transparent=True)
plt.show()
...
blue_h2_emissions(Direct_emissions_grey, capture_rate_high, Drive_CO2_emissions_grams_grey, capture_rate_flue_gas_high, Indirect_emissions_NOR_base, leakage_rate_NOR_Policy, Total_methane_SMR, GWP20_CH4)
...
ax = fig.add_subplot(1, 2, 1)
ax2 = ax.secondary_yaxis('right', functions=(lambda MJ: MJ*120/1000, lambda kg: kg/120))
ax2.set_yticks(np.arange(0,37,6))
...
plt.xlim(0,0.1)
plt.ylim(0,300)
plt.ylabel('LEOH $[g CO_2eq/MJ H_2]$')
...
ax5.set_ylabel(ylabel= '$[kg CO_2eq/kg H_2]$')
...
colors = ['darkblue', 'cornflowerblue', 'grey', 'hotpink', 'red']
#baseline
plt.subplot(1,2,1)
...
plt.stackplot(x,data, colors=colors)
plt.grid(True, axis='y')
plt.grid(True, axis='x')
ax.set_axisbelow(True)
plt.legend(bbox_to_anchor = (0,1.02,1.02,0), loc = 'lower left', mode = 'expand')
plt.xlim(2025,2050)
plt.ylim(0,4)
plt.ylabel('LCOH [€/kg H2]')
...
#policy
plt.subplot(1,2,2)
plt.plot(LCOH_blue_NOR_Policy, color ='blue', linestyle= '-')
plt.plot(LCOH_green_NOR_on1, color ='green', linestyle ='--')
plt.plot(LCOH_green_NOR_Base, color ='green', linestyle ='-')
plt.plot(LCOH_green_NOR_off1, color ='green', linestyle =':')
...
opex_y = float(tea_blue.loc['Opex [€/kW/a]'][sensi_year])
...
plt.plot(LCOH_green_NOR_Policy, color='green', linestyle='--', label = 'LCOH Green Policy')
plt.plot(price_sensitivities, color='blue', linestyle='solid', label = 'LCOH Blue Baseline')
plt.plot(policy.price_sensitivities, color='blue', linestyle='--', label = 'LCOH Blue Policy')
...
plt.ylim(0,6.5)
ax.set_axisbelow(True)
plt.xticks([0,7.5,15,22.5,30], ['0','7.5', '15', '22.5', '30'])
plt.ylabel('LCOH [€/kg H2]')
...
title = 'LCOH_blue_lifetime_capture_sensi'
output_file = os.path.join(path_plt,title)
plt.savefig(output_file+'.png', transparent = True)
...
el_recon_y = float(tea_lh2.loc['Reconversion - Electricity consumption opt. [kWh/kg H2]'][year])
P_co2_y = float(prices.loc['CO2 prices [€/t_CO2] Baseline'][year])
p_el_y = 50#float(prices.loc['Electricity prices in Norway [€_2021/MWh] Baseline'][year])
...
#Impact of cargo emissions on transport costs
 #float(prices.loc['Electricity prices in Norway [€_2021/MWh] Baseline'][year])
LEOH_LCOT = np.arange(0, 10.1, 1)
d_sea_sensi = 10000
P_co2_y = float(prices.loc['CO2 prices [€/t_CO2] Policy'][year])
sensitivity = []
def lh2_transport_distance(p_el_y, alpha_liq, alpha_et, alpha_ship, alpha_it, alpha_recon,capex_liq_y, capex_et_y,capex_ship_y, capex_it_y,  bog_ship, capex_recon_y, d_sea_sensi ,v_ship, f_ship, el_et ,el_it ,t_it ,el_recon_y):

    for H2_emissions in LEOH_LCOT:

        result = \
        ((alpha_liq * capex_liq_y/1000 + opex_liq_y/1000) + el_liq_y * p_el_y/1000)\
        + (alpha_et * capex_et_y/1000 + opex_et_y/1000) + (el_et + el_reliq * t_et) * p_el_y/1000\
        + (alpha_ship * capex_ship_y + opex_ship_y) / (8760/(2*(d_sea_sensi/v_ship + h_ship)))/(1-(bog_ship * d_sea_sensi/v_ship) - (f_ship  * d_sea_sensi))\
             + (bog_ship * d_sea_sensi/v_ship + f_ship * d_sea_sensi) * (lcoh + H2_emissions * P_co2_y / 1000)\
        + (alpha_it * capex_it_y/1000 + opex_it_y/1000) + (el_it + el_reliq * t_it) * p_el_y/1000\
        + (alpha_recon * capex_recon_y / 1000 + opex_recon_y / 1000) + el_recon_y * p_el_y / 1000\
...
lh2_transport_distance(p_el_y, alpha_liq, alpha_et, alpha_ship, alpha_it, alpha_recon, capex_liq_y, capex_et_y,
                       capex_ship_y, capex_it_y, bog_ship, capex_recon_y, d_sea_sensi, v_ship, f_ship, el_et, el_it, t_it,
                       el_recon_y)

lh2_transport_sensi_cargo_emissions_LCOT_pol= pd.DataFrame(sensitivity, LEOH_LCOT, columns=['LH2 transport costs [€/kg H2]'])
lh2_transport_sensi_cargo_emissions_LCOT_pol.index.name = 'Hydrogen production emissions [kg CO2eq/kg H2]'
lh2_transport_sensi_cargo_emissions_LCOT_pol
#base
sensitivity = []
P_co2_y = float(prices.loc['CO2 prices [€/t_CO2] Baseline'][year])
def lh2_transport_distance(p_el_y, alpha_liq, alpha_et, alpha_ship, alpha_it, alpha_recon,capex_liq_y, capex_et_y,capex_ship_y, capex_it_y,  bog_ship, capex_recon_y, d_sea_sensi ,v_ship, f_ship, el_et ,el_it ,t_it ,el_recon_y):
    ...
    for H2_emissions in LEOH_LCOT:
        ...
        ((alpha_liq * capex_liq_y/1000 + opex_liq_y/1000) + el_liq_y * p_el_y/1000)\
        + (alpha_et * capex_et_y/1000 + opex_et_y/1000) + (el_et + el_reliq * t_et) * p_el_y/1000\
        + (alpha_ship * capex_ship_y + opex_ship_y) / (8760/(2*(d_sea_sensi/v_ship + h_ship)))\
             /(1-(bog_ship * d_sea_sensi/v_ship) - (f_ship  * d_sea_sensi))\
             + (bog_ship * d_sea_sensi/v_ship + f_ship * d_sea_sensi) * (lcoh)\
        + (alpha_it * capex_it_y/1000 + opex_it_y/1000) + (el_it + el_reliq * t_it) * p_el_y/1000\
        + (alpha_recon * capex_recon_y / 1000 + opex_recon_y / 1000) + el_recon_y * p_el_y / 1000\
...
lh2_transport_sensi_cargo_emissions_LCOT_base= pd.DataFrame(sensitivity, LEOH_LCOT, columns=['LH2 transport costs [€/kg H2]'])
lh2_transport_sensi_cargo_emissions_LCOT_base.index.name = 'Hydrogen production emissions [kg CO2eq/kg H2]'
lh2_transport_sensi_cargo_emissions_LCOT_base
"""## NH3 Transport"""
...
plt.ylabel('LCOT [€/kg H2]')
#plt.xlabel('Policy', fontweight='bold')
plt.title('NH3 Policy')
title = '\Ammonia_cost_breakdown_stackplot_base_pol'
plt.savefig(path_plt + title + '.png', transparent = True)
plt.show()

# ammonia cost break down stacked w/o cracking base pol
#nh3 cost breakdown base pol
#read pol
...
plt.legend(loc='upper right')
plt.ylabel('LCOT [€/kg H2]')
plt.title('NH3 w/o cracking Baseline')
...
plt.xlim(2025,2050)
plt.ylabel('LCOT [€/kg H2]')
plt.title('NH3 w/o cracking Policy')
...
sensitivity = []
def nh3_transport_sensi_distance_wo_recon(p_el_y, alpha_con, alpha_et, alpha_ship, alpha_it, alpha_recon,capex_con_y, capex_et_y,capex_ship_y, capex_it_y,  bog_ship_nh3, capex_recon_y, d_sea ,v_ship, f_ship_nh3, el_et ,el_it ,t_it ,el_recon_y):
...
nh3_transport_sensi_distance_wo_recon(p_el_y, alpha_con, alpha_et, alpha_ship, alpha_it, alpha_recon,capex_con_y, capex_et_y,capex_ship_y, capex_it_y,  bog_ship_nh3, capex_recon_y, d_sea ,v_ship, f_ship, el_et ,el_it ,t_it ,el_recon_y)

NH3_wo_recon_transport_sensi_distance_LCOT= pd.DataFrame(sensitivity, transport_distance, columns=['NH3 transport costs w/o recon [€/kg H2]'])
NH3_wo_recon_transport_sensi_distance_LCOT.index.name = 'Transport distance in km'
NH3_wo_recon_transport_sensi_distance_LCOT
"""## Sensitivity Plots
...
plt.plot(NH3_wo_recon_transport_sensi_distance_LCOT, color='darkorange', linestyle='--', label ='NH3 w/o cracking')
...
plt.text(d_sea, top, 'Norway Kårstø', horizontalalignment='center', verticalalignment='center')
plt.text(d_sea_hammerfest,top,  'Norway Hammerfest', horizontalalignment='center', verticalalignment='center')
plt.text(d_sea_US,top,  'USA Texas', horizontalalignment='center', verticalalignment='center')
plt.grid(True, axis='y')
#plt.grid(True, axis='x')
ax.set_axisbelow(True)
...
ax.set_axisbelow(True)
plt.legend(loc='upper right')
plt.ylabel('LEOT $[g CO_2eq/kg H_2]$')
plt.title('LH2 Baseline')
plt.subplot(1,2,2)
#policy
fig.patch.set_visible(False)
...
year = 2030
el_liq_y = float(tea_lh2.loc['Liquefaction - Electricity consumption opt. [kWh/kgH2]'][year])
EF_y_n_sensi = 200# float(GHG.loc['Grid electricity emission factor NOR [g CO2eq/kWh] Baseline'][year])

el_recon_y = float(tea_lh2.loc['Reconversion - Electricity consumption opt. [kWh/kg H2]'][year])
#EF_y_G = float(GHG.loc['Grid electricity emission factor GER [g CO2eq/kWh] Baseline'][year])
H2_emissions_sensi = 2500
LH2_cargo_ghg_sensi = (H2_emissions_sensi + (el_liq_y * EF_y_n_sensi ) + ((el_et + el_reliq * t_et) * EF_y_n_sensi ))
sensitivity = []
def lh2_transport_sensi(el_liq_y, EF_y_n_sensi, el_et, el_reliq, t_et, bog_ship, d_sea ,v_ship, f_ship, LH2_cargo_ghg ,el_it ,t_it ,el_recon_y):

    for EF_y_n_sensi in EF_sensi_range:

        result = \
        (el_liq_y * EF_y_n_sensi )\
        + ((el_et + el_reliq * t_et) * EF_y_n_sensi) \
        + ((bog_ship * d_sea_sensi / v_ship + f_ship * d_sea_sensi) * LH2_cargo_ghg_sensi)\
        + ((el_it + el_reliq * t_it) * EF_y_n_sensi )\
        + (el_recon_y * EF_y_n_sensi )
...
lh2_transport_sensi(el_liq_y, EF_y_n_sensi, el_et, el_reliq, t_et, bog_ship, d_sea, v_ship, f_ship, LH2_cargo_ghg, el_it,
                    t_it, el_recon_y)
...
cargo_emissions_range = np.arange(0, 10001, 1000)
d_sea_sensi = 10000
sensitivity = []
def lh2_transport_sensi(el_liq_y, EF_y_n_sensi, el_et, el_reliq, t_et, bog_ship, d_sea ,v_ship, f_ship, LH2_cargo_ghg ,el_it ,t_it ,el_recon_y):
    ...
    for H2_sensi_emissions in cargo_emissions_range:
        ...
        (el_liq_y * EF_sensi_EU_2030)\
        + ((el_et + el_reliq * t_et) * EF_sensi_EU_2030)\
        + ((bog_ship * d_sea_sensi / v_ship + f_ship * d_sea_sensi) * ((el_liq_y * EF_sensi_EU_2030) + ((el_et + el_reliq * t_et) * EF_sensi_EU_2030) + H2_sensi_emissions))\
        + ((el_it + el_reliq * t_it) * EF_sensi_EU_2030)\
        + (el_recon_y * EF_sensi_EU_2030)
...
lh2_transport_sensi_distance_LEOT = pd.DataFrame(sensitivity, columns=['LH2 transport emission [g CO2eq/kg H2]'])
...
el_recon_y = float(tea_NH3.loc['Reconversion - Electricity consumption opt. [kWh/kg H2]'][year])
heat_recon_y = float(tea_NH3.loc['Reconversion - Heat consumption opt. [kWh/kg H2]'][year])
EF_y_n_sensi  # float(GHG.loc['Grid electricity emission factor NOR [g CO2eq/kWh] Baseline'][year])

NH3_cargo_ghg_sensi = (H2_emissions_sensi + (el_con_y * EF_y_n_sensi ) + ((el_et_nh3 + el_reliq_nh3 * (NH3_lhv/H2_lhv) * t_et) * EF_y_n_sensi ))
sensitivity = []

def NH3_sensi_w_recon(el_con_y, EF_y_n_sensi, el_et_nh3, el_reliq_nh3, t_et, bog_ship_nh3, d_sea, v_ship, f_ship_nh3, NH3_cargo_ghg, el_it, heat_recon_y, t_it, el_recon_y):

    for EF_y_n_sensi in EF_sensi_range:

        result =\
            (el_con_y * EF_y_n_sensi )\
        + ((el_et_nh3 + el_reliq_nh3 * (NH3_lhv/H2_lhv) * t_et) * EF_y_n_sensi )\
        + ((bog_ship_nh3 * d_sea_sensi / v_ship + f_ship_nh3 * d_sea_sensi) * NH3_cargo_ghg_sensi)\
        + ((el_it + el_reliq * (NH3_lhv/H2_lhv) * t_it) * EF_y_n_sensi )\
        + ((heat_recon_y + el_recon_y) * EF_y_n_sensi )
...
NH3_sensi_w_recon(el_con_y, EF_y_n_sensi, el_et_nh3, el_reliq_nh3, t_et, bog_ship_nh3, d_sea, v_ship, f_ship_nh3, NH3_cargo_ghg,
...
sensitivity = []
def NH3_sensi_wo_recon(el_con_y, EF_y_n_sensi, el_et_nh3, el_reliq_nh3, t_et, bog_ship_nh3, d_sea, v_ship, f_ship_nh3, NH3_cargo_ghg, el_it, heat_recon_y, t_it, el_recon_y):
...
# recall function
NH3_sensi_wo_recon(el_con_y, EF_y_n_sensi, el_et_nh3, el_reliq_nh3, t_et, bog_ship_nh3, d_sea, v_ship, f_ship_nh3, NH3_cargo_ghg,
                   el_it, heat_recon_y, t_it, el_recon_y)
...
def NH3_sensi_w_recon(el_con_y, EF_y_n_sensi, el_et_nh3, el_reliq_nh3, t_et, bog_ship_nh3, d_sea, v_ship, f_ship_nh3, NH3_cargo_ghg, el_it, heat_recon_y, t_it, el_recon_y):
    ...
    for H2_sensi_emissions in cargo_emissions_range:
        ...
        result =\
            (el_con_y * EF_sensi_EU_2030)\
        + ((el_et_nh3 + el_reliq_nh3 * (NH3_lhv/H2_lhv) * t_et) * EF_sensi_EU_2030)\
        + ((bog_ship_nh3 * d_sea_sensi / v_ship + f_ship_nh3 * d_sea_sensi) * ((el_con_y * EF_sensi_EU_2030) + ((el_et_nh3 + el_reliq_nh3 * (NH3_lhv / H2_lhv) * t_et) * EF_sensi_EU_2030) + H2_sensi_emissions))\
        + ((el_it + el_reliq * (NH3_lhv/H2_lhv) * t_it) * EF_sensi_EU_2030)\
        + ((heat_recon_y + el_recon_y) * EF_sensi_EU_2030)
...
NH3_sensi_w_recon_LEOT = pd.DataFrame(sensitivity, columns=['NH3 transport emission w/o recon [g CO2eq/kg H2]'])
...
sensitivity = []
def nh3_transport_sensi(p_el_y_sensi, alpha_con, alpha_et, alpha_ship, alpha_it, alpha_recon,capex_con_y, capex_et_y,capex_ship_y, capex_it_y,  bog_ship_nh3, capex_recon_y, d_sea_sensi ,v_ship, f_ship_nh3, el_et ,el_it ,t_it ,el_recon_y):

    for H2_emissions in LEOH_LCOT:

        result = \
            (alpha_con * capex_con_y/1000 + opex_con_y/1000) + el_con_y * p_el_y_sensi / 1000 \
            + (alpha_et * capex_et_y/1000 + opex_et_y/1000) + (el_et + el_reliq * (NH3_lhv/H2_lhv) * t_et) * p_el_y_sensi / 1000 \
            + (alpha_ship * capex_ship_y + opex_ship_y)/(8760/(2*(d_sea_sensi/v_ship + h_ship)))\
             /(1-(bog_ship_nh3 * d_sea_sensi/v_ship) - (f_ship_nh3 * d_sea_sensi))\
            + (bog_ship_nh3 * d_sea_sensi/v_ship + f_ship_nh3 * d_sea_sensi) * (policy.NH3_costs_y + H2_emissions * P_co2_y / 1000 ) \
            + (alpha_it * capex_it_y/1000 + opex_it_y/1000) + (el_it + el_reliq * (NH3_lhv/H2_lhv) * t_it) * p_el_y_sensi / 1000 \
            + (alpha_recon * capex_recon_y / 1000 + opex_recon_y / 1000) + (heat_recon_y + el_recon_y) * p_el_y_sensi / 1000

        sensitivity.append(result)

    return sensitivity

nh3_transport_sensi(p_el_y_sensi, alpha_con, alpha_et, alpha_ship, alpha_it, alpha_recon,capex_con_y, capex_et_y,capex_ship_y, capex_it_y,  bog_ship_nh3, capex_recon_y, d_sea_sensi ,v_ship, f_ship_nh3, el_et ,el_it ,t_it ,el_recon_y)

nh3_transport_sensi_cargo_emissions_LCOT_pol= pd.DataFrame(sensitivity, LEOH_LCOT, columns=['NH3 transport costs [€/kg H2]'])
nh3_transport_sensi_cargo_emissions_LCOT_pol.index.name = 'Hydrogen production emissions [kg CO2eq/kg H2]'
nh3_transport_sensi_cargo_emissions_LCOT_pol
#base
P_co2_y = float(prices.loc['CO2 prices [€/t_CO2] Baseline'][year])
sensitivity = []
def nh3_transport_sensi(p_el_y_sensi, alpha_con, alpha_et, alpha_ship, alpha_it, alpha_recon,capex_con_y, capex_et_y,capex_ship_y, capex_it_y,  bog_ship_nh3, capex_recon_y, d_sea_sensi ,v_ship, f_ship_nh3, el_et ,el_it ,t_it ,el_recon_y):
    ...
    for H2_emissions in LEOH_LCOT:
        ...
        result = \
            ...
            + (alpha_ship * capex_ship_y + opex_ship_y)/(8760/(2*(d_sea_sensi/v_ship + h_ship)))\
             /(1-(bog_ship_nh3 * d_sea_sensi/v_ship) - (f_ship_nh3 * d_sea_sensi))\
             + (bog_ship_nh3 * d_sea_sensi/v_ship + f_ship_nh3 * d_sea_sensi) * (NH3_costs_y) \
            + (alpha_it * capex_it_y/1000 + opex_it_y/1000) + (el_it + el_reliq * (NH3_lhv/H2_lhv) * t_it) * p_el_y_sensi / 1000 \
            + (alpha_recon * capex_recon_y / 1000 + opex_recon_y / 1000) + (heat_recon_y + el_recon_y) * p_el_y_sensi / 1000
...
nh3_transport_sensi_cargo_emissions_LCOT_base= pd.DataFrame(sensitivity, LEOH_LCOT, columns=['NH3 transport costs [€/kg H2]'])
nh3_transport_sensi_cargo_emissions_LCOT_base.index.name = 'Hydrogen production emissions [kg CO2eq/kg H2]'
nh3_transport_sensi_cargo_emissions_LCOT_base
...
fig, ax = plt.subplots(figsize=(8,3.2), layout = 'constrained')
plt.subplot(1,2,1)
plt.plot(lh2_transport_sensi_distance_LEOT.div(1000), color='blue', linestyle='-', label = 'LH2')
plt.plot(NH3_sensi_w_recon_LEOT.div(1000), color='darkorange', linestyle='-', label ='NH3')

plt.grid(True, axis='y')
#plt.grid(True, axis='x')
ax.set_axisbelow(True)

plt.ylabel('LEOT $[kg CO_2eq/kg H_2]$')
plt.xlim(0,10)
#plt.ylim(0,)
plt.xlabel('LEOH $[kg CO_2eq/kg H_2]$')
plt.legend()
...
plt.plot(nh3_transport_sensi_cargo_emissions_LCOT_base, color='darkorange', linestyle='-', label ='NH3 Policy')
plt.plot(nh3_transport_sensi_cargo_emissions_LCOT_pol, color='darkorange', linestyle='--', label ='NH3 Baseline')
...
plt.xlabel('LEOH $[kg CO_2eq/kg H_2]$')
plt.legend()
title = '\Transport_sensi_cargo_emissions_LEOT_LCOT'
plt.savefig(path_plt + title + '.png', transparent=True)
...
plt.locator_params(axis='x', nbins=12)
plt.ylabel('LEOT $[g CO_2eq/kg H_2]$')
plt.xlabel('Electricity emission factor $[g CO_2eq/kWh]$')
#plt.ylim(0,3000)
plt.legend(loc = 'upper left')
...
d_sea = d_sea_US
p_el_y = 50#float(prices.loc['Electricity prices in Norway [€_2021/MWh] Baseline'][year])
EF_y_n = 200 #float(GHG.loc['Grid electricity emission factor NOR [g CO2eq/kWh] Baseline'][year])
# LCOH blue
year = 2030
CF = float(tea_blue.loc['Availability [%]']['NGR with CCS'])
# Plant efficiency in %
...
#Lh2 baseline LEOT
CE = Conversion_emissions_LH2()/1000
EE = ET_emissions_LH2()/1000
LH2_cargo_ghg = CE + EE + Q_ue_y
SE = calculate_ship_emissions_LH2()
IE = IT_emissions_LH2()/1000
RE = calculate_recon_emissions_LH2()/1000

x = 0
RE_plt = plt.bar(x ,RE,  color = 'orange', bottom= CE + EE + SE + IE, label = 'Reconversion')
IE_plt = plt.bar(x ,IE,  color = 'wheat', bottom= CE + EE + SE, label = 'Import terminal')
...
plt.plot(x, Q_ue_y, marker = 'o',color = 'blue', label = 'LEOH Blue Baseline')
x=0.5
plt.plot(x, 0, marker = ">", color = 'green', label = 'LEOH Green')

#Lh2 policy LEOT
CE = policy.CE_US_LH2/1000
EE = policy.EE_US_LH2/1000
SE = policy.SE_US_LH2/1000
IE = policy.IE_US_LH2/1000
RE = policy.RE_US_LH2/1000
x = 1
RE_plt = plt.bar(x ,RE,  color = 'orange', bottom= CE + EE + SE + IE)
IE_plt = plt.bar(x ,IE,  color = 'wheat', bottom= CE + EE + SE)
SE_plt = plt.bar(x ,SE,  color = 'crimson', bottom= CE + EE)
EE_plt = plt.bar(x ,EE, color = 'cornflowerblue', bottom= CE)
CE_plt = plt.bar(x ,CE, color = 'darkblue')
plt.plot(x, policy.Q_ue_y, marker = 'D', color = 'blue', label = 'LEOH Blue Policy')
#Nh3 baseline LEOT
el_con_y = float(tea_NH3.loc['Conversion - Electricity consumption opt. [kWh/kgH2]'][year])
el_recon_y = float(tea_NH3.loc['Reconversion - Electricity consumption opt. [kWh/kg H2]'][year])
CE = Conversion_emissions_NH3()/1000
EE = ET_emissions_NH3()/1000
NH3_cargo_ghg = CE + EE + Q_ue_y
SE = calculate_ship_emissions_NH3()/1000
IE = IT_emissions_NH3()/1000
RE = calculate_recon_emissions_NH3()/1000
x = 2
RE_plt = plt.bar(x ,RE,  color = 'orange', bottom= CE + EE + SE + IE)
IE_plt = plt.bar(x ,IE,  color = 'wheat', bottom= CE + EE + SE)
SE_plt = plt.bar(x ,SE,  color = 'crimson', bottom= CE + EE)
...
#Nh3 policy LEOT
CE = policy.CE_US_NH3/1000
EE = policy.EE_US_NH3/1000
SE = policy.SE_US_NH3/1000
IE = policy.IE_US_NH3/1000
RE = policy.RE_US_NH3/1000
x = 3
RE_plt = plt.bar(x ,RE,  color = 'orange', bottom= CE + EE + SE + IE)
IE_plt = plt.bar(x ,IE,  color = 'wheat', bottom= CE + EE + SE)
SE_plt = plt.bar(x ,SE,  color = 'crimson', bottom= CE + EE)
...
LC = calculate_liq_costs_LH2(alpha_liq, capex_liq_y, opex_liq_y, el_liq_y, p_el_y)
EC = calculate_export_terminal_costs_LH2(alpha_et, capex_et_y, opex_et_y, el_et, p_el_y, el_reliq)
lcoh = (LC + EC + lcoh_blue_sensi_US)
SC = calculate_ship_costs_LH2()
IC = calculate_import_terminal_costs_LH2()
RC = calculate_recon_costs_LH2(alpha_recon, capex_recon_y, opex_recon_y, el_recon_y, p_el_y)

x = 0
plt.bar(x,RC , color= 'orange',bottom= LC + EC + SC + IC)
plt.bar(x,IC , color= 'wheat',bottom= LC + EC + SC)
plt.bar(x,SC , color= 'crimson', bottom= LC + EC)
plt.bar(x,EC , color= 'cornflowerblue',bottom= LC)
plt.bar(x,LC , color= 'darkblue')
plt.plot(x, lcoh_blue_sensi_US, marker = 'o', color = 'blue', label = 'LCOH Blue Baseline')

x = 0.5
plt.plot(x, LCOH_green_US, marker = ">", color = 'green', label = 'LCOH Green (PV)')
...
#LCOT NH3 baseline
CC = calculate_con_costs_NH3()
EC = calculate_export_terminal_costs_NH3()
NH3_costs_y = ( CC + EC + lcoh_blue_sensi_US)
SC = calculate_ship_costs_NH3(alpha_ship, capex_ship_y, opex_ship_y, d_sea, v_ship, h_ship, bog_ship_nh3, f_ship_nh3, NH3_costs_y)
IC = calculate_import_terminal_costs_NH3(alpha_it, capex_it_y, opex_it_y, el_it, p_el_y, t_it)
RC = calculate_recon_costs_NH3(alpha_recon, capex_recon_y, opex_recon_y, el_recon_y, p_el_y)
x = 2
plt.bar(x,RC , color= 'orange',bottom= CC + EC + SC + IC)
plt.bar(x,IC , color= 'wheat',bottom= CC + EC + SC)
plt.bar(x,SC , color= 'crimson', bottom= CC + EC)
plt.bar(x,EC , color= 'cornflowerblue',bottom= CC)
plt.bar(x,CC , color= 'darkblue')
...
plt.axhline(RED_tresh_kg,0, color = 'lime', linestyle = '-', marker = 'o', label = 'EU Comission Treshold')
plt.axvline(x=av_new, color='dodgerblue', linestyle = '--')
plt.axvline(x=av_retro, color='royalblue', linestyle = '--')
plt.text(av_new, top, 'New', horizontalalignment='center', verticalalignment='center')
plt.text(av_retro,top,  'Retrofit', horizontalalignment='center', verticalalignment='center')

plt.ylabel('Supply emissions $[kg CO_2eq/kg H_2]$')
plt.xlabel('Baseline', fontweight='bold')
plt.xlim(2025,2050)
plt.ylim(0,ylim)
plt.legend()
